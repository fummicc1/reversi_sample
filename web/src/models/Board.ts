...
export const flipDisk = (board: Board, position: Position, myColor: Disk): Board => {
    ...
    for (const xDiff of xNeighbors) {
        for (const yDiff of yNeighbors) {
            ...
            const can = flipDiskRecursive(tempBoard, position, xDiff, yDiff, myColor, opponentColor);
...
const flipDiskRecursive = (board: Board, position: Position, xDiff: number, yDiff: number, target: Disk, interchangeable: Disk): Position[] => {
    let tailPosition: Position | null = {
        x: position.x + xDiff,
        y: position.y + yDiff
    }
    if (board.width <= tailPosition.x || tailPosition.x < 0 || board.height <= tailPosition.y || tailPosition.y < 0) {
        return [];
    }
    ...
    if (board.disks[tailPosition.x][tailPosition.y] == interchangeable) {
        if (board.width <= tailPosition.x + xDiff || board.height <= tailPosition.y + yDiff) {
            return [];
        }
        const nextDisk = board.disks[tailPosition.x + xDiff][tailPosition.y + yDiff];
        if (nextDisk == target) {
        ...
        board.disks[tailPosition.x][tailPosition.y] = target;
        let ans: Position[] = []
        const rec = flipDiskRecursive(board, tailPosition, xDiff, yDiff, target, interchangeable)
        if (rec.length) {
